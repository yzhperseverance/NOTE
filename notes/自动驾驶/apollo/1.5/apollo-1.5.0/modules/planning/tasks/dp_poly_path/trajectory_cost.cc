...
double TrajectoryCost::Calculate(const QuinticPolynomialCurve1d &curve,
  ...
  while (end_index != 0) {
    // 为啥这里要用speed point？？这里的heuristic_speed_data_应该是预计这一帧的速度，然后用它计算每个时刻曲线上点的位置，
    // 可能是为了减少计算量？因为直接在曲线上采样好像也没啥区别
    common::SpeedPoint speed_point;
    heuristic_speed_data_.EvaluateByTime(time_stamp, &speed_point); // 这个速度是哪来的？？？往前找没找到
  ...
  for (; start_index < end_index; ++start_index) {
    ...
    sl.set_l(l);
    reference_line_->SLToXY(sl, &ego_xy_point); // 转到笛卡尔坐标系下以计算跟障碍物的距离
