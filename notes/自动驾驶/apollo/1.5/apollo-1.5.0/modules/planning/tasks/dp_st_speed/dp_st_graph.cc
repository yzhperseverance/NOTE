...
void DpStGraph::CalculateCostAt(const uint32_t c, const uint32_t r) {
  /*** 由于差分法计算jerk至少要4个点，所以前三个点单独计算cost  ***/
  ...
  if (c == 1) {
    // total_cost默认是无穷大，所以如果点与障碍物重合，直接return掉保留无穷大即可
...
Status DpStGraph::RetrieveSpeedProfile(SpeedData* const speed_data) const {
  ...
  // cost_table_.back()是最后时刻的s个point
  // 从时间维度上，最后时刻代表终点，计算出最后时刻的s个点的最小代价
  ...
  // 从空间维度上，最后位置代表终点，计算每行t最后一个s的最小值
  /*** 这两个终点其实是不一样的，在空间角度上最后位置的s相同，而在时间角度上
   *   最后时刻的不同位置则代表车辆的运行速度不同，从0到s速度是逐渐变快的，因为时间相同s递增
   ***/
